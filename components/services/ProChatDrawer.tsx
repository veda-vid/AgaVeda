// components/services/ProChatDrawer.tsx
...
import { getOrCreateProConversation, getProMessages, sendProMessage } from '../../lib/api';
...
type ProChatDrawerProps = {
  pro: ServiceProvider;
  userId: string;
  onClose: () => void;
};

export function ProChatDrawer({ pro, userId, onClose }: ProChatDrawerProps) {
  ...
  useEffect(() => {
    ...
    getOrCreateProConversation(userId, pro)
      .then(async conv => {
        ...
        setMessages(await getProMessages(conv.id));
      })
      .catch(console.error)
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        event: 'INSERT', schema: 'public', table: 'pro_messages',
    ...
    <Modal visible animationType="slide" transparent onRequestClose={onClose}>
      <KeyboardAvoidingView style={s.overlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <View style={s.sheet}>
          ...
          <View style={s.header}>
            <View style={{ flex: 1 }}>
              <Text style={[s.title, unicodeProsStyle]}>{pro.business_name}</Text>
              <Text style={s.sub}>{getProCategoryLabel(pro)}</Text>
            </View>
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <View style={[s.bubble, mine ? s.bubbleMine : s.bubbleTheirs]}>
                    <Text style={[s.bubbleText, mine && s.bubbleTextMine, unicodeProsStyle]}>{item.body}</Text>
                  </View>
                );
              }}
              ListEmptyComponent={
                <Text style={s.emptyChat}>Say hello — ask about availability or pricing.</Text>
              }
            />
          )}
