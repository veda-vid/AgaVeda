-- Unified Sparks feed: merge reels + video posts with hydrated interaction state

ALTER TABLE public.reels
  ADD COLUMN IF NOT EXISTS product_id UUID REFERENCES public.products(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_reels_product ON public.reels(product_id);
...
GRANT EXECUTE ON FUNCTION public.get_unified_sparks_feed(UUID, INT, INT, UUID[]) TO authenticated;
